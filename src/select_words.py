...
def main(seed: int, split_seed: int, num_envs: int, episodes: int):
    ...
    for subset, ax in zip(["train", "val"], [ax1, ax2]):
        word_acc = evaluate(guesser, dset, subset, num_envs, episodes)
...
def evaluate(guesser: Guesser, dset: TimitXVectors, subset: str, num_envs: int,
