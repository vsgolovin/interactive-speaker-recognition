...
@click.option("--num-envs", type=int, default=33,
              help="number of ISV environments (games) to run in parallel")
...
@click.option("--lr-actor", type=float, default=5e-5,
              help="PPO actor learning rate")
@click.option("--lr-critic", type=float, default=5e-5,
...
def train(use_codebook: bool, seed: int, split_seed: int, num_words: int,
    ...
    plt.xlabel("Episodes")
    plt.savefig(output_dir / "enquirer4v_training.png", dpi=75)
...
def test(use_codebook: bool, all_subsets: bool, sd_file: str, seed: int,
    ...
    for subset in subsets:
        r = evaluate(ppo, env, subset, num_words, episodes, num_envs, True)
        print(f"Average reward (accuracy) on {subset}: {r}")


def evaluate(ppo: PPO, env: IsvEnvironment, subset: str = "val",
             num_words: int = 3, episodes: int = 20000,
    ...
    while performed < episodes:
        ...
        states = env.reset(subset, batch_size=cur_envs, num_words=num_words)
...
def create_log_dir(root="output/enquirer4v_logs"):
    root = Path(root)
    idx = -1
    if root.exists():
